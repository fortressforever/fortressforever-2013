...
void DropPrimedFragGrenade( CFF_SV_Player *pPlayer, CBaseCombatWeapon *pGrenade );

LINK_ENTITY_TO_CLASS( player, CFF_SV_Player );
...
#define FFPLAYER_PHYSDAMAGE_SCALE 4.0f

#pragma warning( disable : 4355 )

CFF_SV_Player::CFF_SV_Player() : m_PlayerAnimState( this )
{
	m_angEyeAngles.Init();
...
CFF_SV_Player::~CFF_SV_Player( void )
{

}

void CFF_SV_Player::UpdateOnRemove( void )
{
	if ( m_hRagdoll )
	{
...
void CFF_SV_Player::Precache( void )
{
	BaseClass::Precache();
...
void CFF_SV_Player::GiveAllItems( void )
{
	EquipSuit();
...
void CFF_SV_Player::GiveDefaultItems( void )
{
	EquipSuit();
...
void CFF_SV_Player::Spawn(void)
{
	...
	m_impactEnergyScale = FFPLAYER_PHYSDAMAGE_SCALE;
...
void CFF_SV_Player::PickupObject( CBaseEntity *pObject, bool bLimitMassAndSize )
{
	
}

bool CFF_SV_Player::ValidatePlayerModel( const char *pModel )
{
...
void CFF_SV_Player::SetPlayerTeamModel( void )
{
	const char *szModelName = NULL;
	szModelName = engine->GetClientConVarValue( engine->IndexOfEdict( edict() ), "cl_playermodel" );
...
void CFF_SV_Player::SetupPlayerSoundsByModel( const char *pModelName )
{
...
void CFF_SV_Player::ResetAnimation( void )
{
	if ( IsAlive() )
	{
		SetSequence ( -1 );
...
bool CFF_SV_Player::Weapon_Switch( CBaseCombatWeapon *pWeapon, int viewmodelindex )
{
...
void CFF_SV_Player::PreThink( void )
{
...
void CFF_SV_Player::PostThink( void )
{
	BaseClass::PostThink();
...
void CFF_SV_Player::PlayerDeathThink()
{
	if( !IsObserver() )
	{
...
void CFF_SV_Player::FireBullets ( const FireBulletsInfo_t &info )
{
...
void CFF_SV_Player::NoteWeaponFired( void )
{
...
bool CFF_SV_Player::WantsLagCompensationOnEntity( const CBasePlayer *pPlayer, const CUserCmd *pCmd, const CBitVec<MAX_EDICTS> *pEntityTransmitBits ) const
{
...
void CFF_SV_Player::SetAnimation( PLAYER_ANIM playerAnim )
{
	int animDesired;
...
//-----------------------------------------------------------------------------
bool CFF_SV_Player::BumpWeapon( CBaseCombatWeapon *pWeapon )
{
...
void CFF_SV_Player::ChangeTeam( int iTeam )
{
...
void CFF_SV_Player::CheatImpulseCommands( int iImpulse )
{
	switch ( iImpulse )
	{
		case 101:
...
bool CFF_SV_Player::ShouldRunRateLimitedCommand( const CCommand &args )
{
	...
	else if ( (gpGlobals->curtime - m_RateLimitLastCommandTimes[i]) < FF_COMMAND_MAX_RATE )
	{
		// Too fast.
		return false;
	}
...
void CFF_SV_Player::CreateViewModel( int index /*=0*/ )
{
...
bool CFF_SV_Player::BecomeRagdollOnClient( const Vector &force )
{
	return true;
}
...
class CFF_SV_Ragdoll : public CBaseAnimatingOverlay
{
public:
	DECLARE_CLASS( CFF_SV_Ragdoll, CBaseAnimatingOverlay );
	DECLARE_SERVERCLASS();
...
LINK_ENTITY_TO_CLASS( ff_ragdoll, CFF_SV_Ragdoll );

IMPLEMENT_SERVERCLASS_ST_NOBASE( CFF_SV_Ragdoll, DT_FFRagdoll )
...
void CFF_SV_Player::CreateRagdollEntity( void )
{
	if ( m_hRagdoll )
	{
		UTIL_RemoveImmediate( m_hRagdoll );
	...
	CFF_SV_Ragdoll *pRagdoll = dynamic_cast< CFF_SV_Ragdoll* >( m_hRagdoll.Get() );
	
	if ( !pRagdoll )
	{
		// create a new one
		pRagdoll = dynamic_cast< CFF_SV_Ragdoll* >( CreateEntityByName( "ff_ragdoll" ) );
	}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CFF_SV_Player::FlashlightIsOn( void )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CFF_SV_Player::FlashlightTurnOn( void )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CFF_SV_Player::FlashlightTurnOff( void )
{
...
void CFF_SV_Player::Weapon_Drop( CBaseCombatWeapon *pWeapon, const Vector *pvecTarget, const Vector *pVelocity )
{
...
void CFF_SV_Player::DetonateTripmines( void )
{
...
void CFF_SV_Player::Event_Killed( const CTakeDamageInfo &info )
{
...
int CFF_SV_Player::OnTakeDamage( const CTakeDamageInfo &inputInfo )
{
...
void CFF_SV_Player::DeathSound( const CTakeDamageInfo &info )
{
...
CBaseEntity* CFF_SV_Player::EntSelectSpawnPoint( void )
{
...
{
	CFF_SV_Player *pPlayer = ToFFPlayer( UTIL_GetCommandClient() );
...
void CFF_SV_Player::Reset()
{	
	ResetDeathCount();
	ResetFragCount();
}

bool CFF_SV_Player::IsReady()
{
	return m_bReady;
}

void CFF_SV_Player::SetReady( bool bReady )
{
	m_bReady = bReady;
}

void CFF_SV_Player::CheckChatText( char *p, int bufsize )
{
...
void CFF_SV_Player::State_Transition( FFPlayerState newState )
{
	State_Leave();
...
void CFF_SV_Player::State_Enter( FFPlayerState newState )
{
...
void CFF_SV_Player::State_Leave()
{
...
void CFF_SV_Player::State_PreThink()
{
...
CFFPlayerStateInfo *CFF_SV_Player::State_LookupInfo( FFPlayerState state )
{
	// This table MUST match the 
	static CFFPlayerStateInfo playerStateInfos[] =
	{
		{ STATE_ACTIVE,			"STATE_ACTIVE",			&CFF_SV_Player::State_Enter_ACTIVE, NULL, &CFF_SV_Player::State_PreThink_ACTIVE },
		{ STATE_OBSERVER_MODE,	"STATE_OBSERVER_MODE",	&CFF_SV_Player::State_Enter_OBSERVER_MODE,	NULL, &CFF_SV_Player::State_PreThink_OBSERVER_MODE }
	};
...
bool CFF_SV_Player::StartObserverMode(int mode)
{
...
void CFF_SV_Player::StopObserverMode()
{
	m_bEnterObserver = false;
	BaseClass::StopObserverMode();
}

void CFF_SV_Player::State_Enter_OBSERVER_MODE()
{
...
void CFF_SV_Player::State_PreThink_OBSERVER_MODE()
{
...
void CFF_SV_Player::State_Enter_ACTIVE()
{
...
void CFF_SV_Player::State_PreThink_ACTIVE()
{
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CFF_SV_Player::CanHearAndReadChatFrom( CBasePlayer *pPlayer )
{
